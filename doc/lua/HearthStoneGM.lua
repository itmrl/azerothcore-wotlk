print(">>Script: HearthStoneGM loading...OK")

--菜单所有者 --默认炉石6948
local itemEntry = 90003
--阵营
...
function ST.SummonNPC(player, entry)
    ...
    if (player:IsInCombat()) then
        ...
        if (map) then
            ...
            if (nz ~= nil and nz > z and nz < (z + 5)) then
                z = nz
            end
...
-- function ST.SummonGNPC(player)--召唤旅行商人
-- 	ST.SummonNPC(player, ST.NPCID1)
-- end

-- function ST.SummonENPC(player)--召唤旅行修补匠
...
function ST.SummonNPC_190010(player)
    --召唤幻化师
    ST.SummonNPC(player, 190010)
end

function ST.SummonNPC_190012(player)
    --召唤共享材料银行
    ST.SummonNPC(player, 190012)
end

function ST.SummonNPC_190016(player)
    --综合训练师
    ST.SummonNPC(player, 190016)
end

function ST.SummonNPC_AH(player)
    --召唤拍卖师
    local team = player:GetTeam()
    if (team == TEAM_ALLIANCE) then
        ST.SummonNPC(player, 15659)
    elseif (team == TEAM_HORDE) then
        ST.SummonNPC(player, 17629)
    end
end
...
local Stone = {
    ...
    GoHome = function(player)
        ...
        player:SendBroadcastMessage("已经返回了家中。")
    end,
    GoSelectPlayer = function(player)
        --传送到所选队友那里
        ...
        if (target) then
            ...
            if (player:Teleport(mapId, x, y, z, 0, TELE_TO_GM_MODE)) then
                --传送
            ...
            player:SendBroadcastMessage("请选中一个目标。")
        end
    end,
    SetHome = function(player)
        ...
        player:SendBroadcastMessage("已将当前位置设置为你的家。")
    end,
    ...
    -- 	player:SendAuctionMenu(Unit(player))
    -- 	player:SendBroadcastMessage("已经打开拍卖行")
    -- end,

    OpenMailBox = function(player)
        --打开邮箱
    ...
    LevelUp80 = function(player, code)
        --等级直升80
        local target = player:GetSelection()
        if (target and (target:GetTypeId() == player:GetTypeId())) then
            local cl = target:GetClass() --获取玩家的职业
            local lv = target:GetLevel() --获取玩家的等级
            target:SendAreaTriggerMessage("玩家等级：" .. lv)
            if lv < 80 then
                --如果是低于80级的DK
                if cl == 6 then
                    -- 如果是DK
                    ...
                    for k, v in ipairs(STARTER_QUESTS) do
                        local quest_status = target:GetQuestStatus(v)
                        if quest_status == 0 then
                            --没这个任务，自动加这个任务，然后完成
                            target:AddQuest(v)
                            target:CompleteQuest(v)
                            target:RewardQuest(v)
                        end
                    end
                    target:AddItem(38664);
                    target:AddItem(39322);
                    target:AddItem(38632);
                    --target:SetLevel(80) --设置到80级
                    target:SaveToDB() --保存到DB
                else
                    --target:SetLevel(80) --设置到80级
                    target:SaveToDB() --保存到DB
                end
            else
                target:SendAreaTriggerMessage("该玩家已经满级。")
            end

        else
            target:SendAreaTriggerMessage("请选中一个玩家。")
        end
    end,
    ResetName = function(player, code)
        --修改名字
        local target = player:GetSelection()
        if (target and (target:GetTypeId() == player:GetTypeId())) then
    ...
    SY01 = function(player)
        --商业技能熟练度
        local target = player:GetSelection()
        if (target and (target:GetTypeId() == player:GetTypeId())) then
            if target:HasSpell(50310) then
                target:SendBroadcastMessage("你的采矿专业已满。")
                return
            end
            if (target:HasSpell(2575)) then
                target:LearnSpell(2576)
                target:LearnSpell(3564)
                target:LearnSpell(10248)
                target:LearnSpell(29354)
                target:LearnSpell(50310)
                target:AdvanceSkill(186, 450)--1
            else
                target:SendBroadcastMessage("请先学习采矿专业。")
                player:SendAreaTriggerMessage("请先学习采矿专业。")
                return
            end
        else
            player:SendAreaTriggerMessage("请选中一个玩家。")
    ...
    SY02 = function(player)
        --商业技能熟练度
        local target = player:GetSelection()
        if (target and (target:GetTypeId() == player:GetTypeId())) then
            if target:HasSpell(50300) then
                target:SendBroadcastMessage("你的草药学专业已满。")
                return
            end
            if (target:HasSpell(2366)) then
                target:LearnSpell(2368)
                target:LearnSpell(3570)
                target:LearnSpell(11993)
                target:LearnSpell(28695)
                target:LearnSpell(50300)
                target:AdvanceSkill(182, 450)--2
            else
                target:SendBroadcastMessage("请先学习草药学专业。")
                player:SendAreaTriggerMessage("请先学习草药学专业。")
                return
            end
    ...
    SY03 = function(player)
        --商业技能熟练度
        local target = player:GetSelection()
        if (target and (target:GetTypeId() == player:GetTypeId())) then
            if target:HasSpell(50305) then
                target:SendBroadcastMessage("你的剥皮专业已满。")
                return
            end
            if (target:HasSpell(8613)) then
                target:LearnSpell(8617)
                target:LearnSpell(8618)
                target:LearnSpell(10768)
                target:LearnSpell(32678)
                target:LearnSpell(50305)
                target:AdvanceSkill(393, 450)--3
            else
                target:SendBroadcastMessage("请先学习剥皮专业。")
                player:SendAreaTriggerMessage("请先学习剥皮专业。")
                return
            end
    ...
    SY04 = function(player)
        --商业技能熟练度
        local target = player:GetSelection()
        if (target and (target:GetTypeId() == player:GetTypeId())) then
            if target:HasSpell(51306) then
                target:SendBroadcastMessage("你的工程学专业已满。")
                return
            end
            if (target:HasSpell(4036)) then
                target:LearnSpell(4037)
                target:LearnSpell(4038)
                target:LearnSpell(12656)
                target:LearnSpell(30350)
                target:LearnSpell(51306)
                target:AdvanceSkill(202, 450)--4
            else
                target:SendBroadcastMessage("请先学习工程学专业。")
                player:SendAreaTriggerMessage("请先学习工程学专业。")
                return
            end
    ...
    SY05 = function(player)
        --商业技能熟练度
        local target = player:GetSelection()
        if (target and (target:GetTypeId() == player:GetTypeId())) then
            if target:HasSpell(51304) then
                target:SendBroadcastMessage("你的炼金专业已满。")
                return
            end
            if (target:HasSpell(2259)) then
                target:LearnSpell(3101)
                target:LearnSpell(3464)
                target:LearnSpell(11611)
                target:LearnSpell(28596)
                target:LearnSpell(51304)
                target:AdvanceSkill(171, 450)--5
            else
                target:SendBroadcastMessage("请先学习炼金专业。")
                player:SendAreaTriggerMessage("请先学习炼金专业。")
                return
            end
    ...
    SY06 = function(player)
        --商业技能熟练度
        local target = player:GetSelection()
        if (target and (target:GetTypeId() == player:GetTypeId())) then
            if target:HasSpell(51302) then
                target:SendBroadcastMessage("你的制皮专业已满。")
                return
            end
            if (target:HasSpell(2108)) then
                target:LearnSpell(3104)
                target:LearnSpell(3811)
                target:LearnSpell(10662)
                target:LearnSpell(32549)
                target:LearnSpell(51302)
                target:AdvanceSkill(165, 450)--6
            else
                target:SendBroadcastMessage("请先学习制皮专业。")
                player:SendAreaTriggerMessage("请先学习制皮专业。")
                return
            end
    ...
    SY07 = function(player)
        --商业技能熟练度
        local target = player:GetSelection()
        if (target and (target:GetTypeId() == player:GetTypeId())) then
            if target:HasSpell(51309) then
                target:SendBroadcastMessage("你的裁缝专业已满。")
                return
            end
            if (target:HasSpell(3908)) then
                target:LearnSpell(3909)
                target:LearnSpell(3910)
                target:LearnSpell(12180)
                target:LearnSpell(26790)
                target:LearnSpell(51309)
                target:AdvanceSkill(197, 450)--7
            else
                target:SendBroadcastMessage("请先学习裁缝专业。")
                player:SendAreaTriggerMessage("请先学习裁缝专业。")
                return
            end
    ...
    SY08 = function(player)
        --商业技能熟练度
        local target = player:GetSelection()
        if (target and (target:GetTypeId() == player:GetTypeId())) then
            if target:HasSpell(51300) then
                target:SendBroadcastMessage("你的锻造专业已满。")
                return
            end
            if (target:HasSpell(2018)) then
                target:LearnSpell(3100)
                target:LearnSpell(3538)
                target:LearnSpell(9785)
                target:LearnSpell(29844)
                target:LearnSpell(51300)
                target:AdvanceSkill(164, 450)--8
            else
                target:SendBroadcastMessage("请先学习锻造专业。")
                player:SendAreaTriggerMessage("请先学习锻造专业。")
                return
            end
    ...
    SY09 = function(player)
        --商业技能熟练度
        local target = player:GetSelection()
        if (target and (target:GetTypeId() == player:GetTypeId())) then
            if target:HasSpell(51313) then
                target:SendBroadcastMessage("你的附魔专业已满。")
                return
            end
            if (target:HasSpell(7411)) then
                target:LearnSpell(7412)
                target:LearnSpell(7413)
                target:LearnSpell(13920)
                target:LearnSpell(28029)
                target:LearnSpell(51313)
                target:AdvanceSkill(333, 450)--9
            else
                target:SendBroadcastMessage("请先学习附魔专业。")
                player:SendAreaTriggerMessage("请先学习附魔专业。")
                return
            end
    ...
    SY10 = function(player)
        --商业技能熟练度
        local target = player:GetSelection()
        if (target and (target:GetTypeId() == player:GetTypeId())) then
            if target:HasSpell(51311) then
                target:SendBroadcastMessage("你的珠宝专业已满。")
                return
            end
            if (target:HasSpell(25229)) then
                target:LearnSpell(25230)
                target:LearnSpell(28894)
                target:LearnSpell(28895)
                target:LearnSpell(28897)
                target:LearnSpell(51311)
                target:AdvanceSkill(755, 450)--10
            else
                target:SendBroadcastMessage("请先学习珠宝专业。")
                player:SendAreaTriggerMessage("请先学习珠宝专业。")
                return
            end
    ...
    SY11 = function(player)
        --商业技能熟练度
        local target = player:GetSelection()
        if (target and (target:GetTypeId() == player:GetTypeId())) then
            if target:HasSpell(45363) then
                target:SendBroadcastMessage("你的铭文专业已满。")
                return
            end
            if (target:HasSpell(45357)) then
                target:LearnSpell(45358)
                target:LearnSpell(45359)
                target:LearnSpell(45360)
                target:LearnSpell(45361)
                target:LearnSpell(45363)
                target:AdvanceSkill(773, 450)--11
            else
                target:SendBroadcastMessage("请先学习铭文专业。")
                player:SendAreaTriggerMessage("请先学习铭文专业。")
                return
            end
    ...
    SY12 = function(player)
        --商业技能熟练度
        local target = player:GetSelection()
        if (target and (target:GetTypeId() == player:GetTypeId())) then
            if target:HasSpell(51296) then
                target:SendBroadcastMessage("你的烹饪专业已满。")
                return
            end
            if (target:HasSpell(2550)) then
                target:LearnSpell(3102)
                target:LearnSpell(3413)
                target:LearnSpell(18260)
                target:LearnSpell(33359)
                target:LearnSpell(51296)
                target:AdvanceSkill(185, 450)--12
            else
                target:SendBroadcastMessage("请先学习烹饪专业。")
                player:SendAreaTriggerMessage("请先学习烹饪专业。")
                return
            end
    ...
    SY13 = function(player)
        --商业技能熟练度
        local target = player:GetSelection()
        if (target and (target:GetTypeId() == player:GetTypeId())) then
            if target:HasSpell(45542) then
                target:SendBroadcastMessage("你的急救专业已满。")
                return
            end
            if (target:HasSpell(3273)) then
                target:LearnSpell(3274)
                target:LearnSpell(7924)
                target:LearnSpell(10846)
                target:LearnSpell(27028)
                target:LearnSpell(45542)
                target:AdvanceSkill(129, 450)--13
            else
                target:SendBroadcastMessage("请先学习急救专业。")
                player:SendAreaTriggerMessage("请先学习急救专业。")
                return
            end
    ...
    SY14 = function(player)
        --商业技能熟练度
        local target = player:GetSelection()
        if (target and (target:GetTypeId() == player:GetTypeId())) then
            if target:HasSpell(51294) then
                target:SendBroadcastMessage("你的钓鱼专业已满。")
                return
            end
            if (target:HasSpell(7620)) then
                target:LearnSpell(7731)
                target:LearnSpell(7732)
                target:LearnSpell(18248)
                target:LearnSpell(33095)
                target:LearnSpell(51294)
                target:AdvanceSkill(356, 450)--14
            else
                target:SendBroadcastMessage("请先学习钓鱼专业。")
                player:SendAreaTriggerMessage("请先学习钓鱼专业。")
                return
            end
        else
            player:SendAreaTriggerMessage("请选中一个玩家。")
...
local Menu = {
    [MMENU] = {--主菜单
        { MENU, "【|cFFB22222地图传送|r】", TPMENU, GOSSIP_ICON_TAXI },
        { FUNC, "炉石|cFFF0F000→|r回家", Stone.GoHome, GOSSIP_ICON_TAXI, false, "是否回|cFFF0F000家|r ?" },
        { FUNC, "记录|cFFF0F000←|r炉石", Stone.SetHome, GOSSIP_ICON_TAXI, false, "是否将当前当前|cFFF0F000位置|r设置为你的家?" },
        { MENU, "【|cFF0000FF定点传送|r】", TBMENU, GOSSIP_ICON_TAXI },
        { MENU, "升级专业技能", SYMENU, GOSSIP_ICON_TRAINER },
        { FUNC, "综合训练师", ST.SummonNPC_190016, GOSSIP_ICON_TRAINER },
        { MENU, "附魔", ENCMENU, GOSSIP_ICON_TABARD },
        { FUNC, "幻化大师", ST.SummonNPC_190010, GOSSIP_ICON_VENDOR },
        { MENU, "商人", BUYMENU, GOSSIP_ICON_VENDOR },
        { FUNC, "拍卖师", ST.SummonNPC_AH, GOSSIP_ICON_VENDOR },
        { FUNC, "共享材料银行", ST.SummonNPC_190012, GOSSIP_ICON_MONEY_BAG },
        { FUNC, "在线银行", Stone.OpenBank, GOSSIP_ICON_MONEY_BAG },
        { FUNC, "空中邮箱", Stone.OpenMailBox, GOSSIP_ICON_TALK },
        { MENU, "更多|cFF548B54※|r功能", MMENU + 0x10, GOSSIP_ICON_CHAT },
    },
    ...
    [MMENU + 0x10] = {--更多功能
        ...
        { FUNC, "重置宠物天赋", Stone.ResetPetTalents, GOSSIP_ICON_TRAINER, false, "确认重置宠物天赋?" },
        { FUNC, "修理装备", Stone.RepairAll, GOSSIP_ICON_MONEY_BAG, false, "需要花费金币修理装备 ?" },
        { FUNC, "武器熟练度", Stone.WSkillsToMax, GOSSIP_ICON_BATTLE, false, "需要武器熟练度全满?" },
        { FUNC, "回复生命", Stone.MaxHealth, GOSSIP_ICON_INTERACT_1 },
        { FUNC, "脱离战斗", Stone.OutCombat, GOSSIP_ICON_INTERACT_1 },
        { FUNC, "解除虚弱", Stone.WeakOut, GOSSIP_ICON_INTERACT_1, false, "是否解除虚弱，并回复生命 ?", 20000 },
        { FUNC, "目标瞬移", Stone.GoSelectPlayer, GOSSIP_ICON_TAXI, false, "是否瞬移到目标身边 ?" },
    },

    [SYMENU] = {
        { FUNC, "提升熟练度->|cFF006400【采矿】|r->|cFFB22222450|r", Stone.SY01, GOSSIP_ICON_TRAINER, false, "是否提升|cFFFFFF00采矿|r技能？", 0 }, --增加收费金额参数,默认情况下失败也会扣金币，因此在失败时返还金币，在游戏里也不会出现减钱再加钱
        { FUNC, "提升熟练度->|cFF006400【草药】|r->|cFFB22222450|r", Stone.SY02, GOSSIP_ICON_TRAINER, false, "是否提升|cFFFFFF00草药|r技能？", 0 },
        { FUNC, "提升熟练度->|cFF006400【剥皮】|r->|cFFB22222450|r", Stone.SY03, GOSSIP_ICON_TRAINER, false, "是否提升|cFFFFFF00剥皮|r技能？", 0 },
        { FUNC, "提升熟练度->|cFF006400【工程】|r->|cFFB22222450|r", Stone.SY04, GOSSIP_ICON_TRAINER, false, "是否提升|cFFFFFF00工程|r技能？", 0 },
        { FUNC, "提升熟练度->|cFF006400【炼金】|r->|cFFB22222450|r", Stone.SY05, GOSSIP_ICON_TRAINER, false, "是否提升|cFFFFFF00炼金|r技能？", 0 },
        { FUNC, "提升熟练度->|cFF006400【制皮】|r->|cFFB22222450|r", Stone.SY06, GOSSIP_ICON_TRAINER, false, "是否提升|cFFFFFF00制皮|r技能？", 0 },
        { FUNC, "提升熟练度->|cFF006400【裁缝】|r->|cFFB22222450|r", Stone.SY07, GOSSIP_ICON_TRAINER, false, "是否提升|cFFFFFF00裁缝|r技能？", 0 },
        { FUNC, "提升熟练度->|cFF006400【锻造】|r->|cFFB22222450|r", Stone.SY08, GOSSIP_ICON_TRAINER, false, "是否提升|cFFFFFF00锻造|r技能？", 0 },
        { FUNC, "提升熟练度->|cFF006400【附魔】|r->|cFFB22222450|r", Stone.SY09, GOSSIP_ICON_TRAINER, false, "是否提升|cFFFFFF00附魔|r技能？", 0 },
        { FUNC, "提升熟练度->|cFF006400【珠宝】|r->|cFFB22222450|r", Stone.SY10, GOSSIP_ICON_TRAINER, false, "是否提升|cFFFFFF00珠宝|r技能？", 0 },
        { FUNC, "提升熟练度->|cFF006400【铭文】|r->|cFFB22222450|r", Stone.SY11, GOSSIP_ICON_TRAINER, false, "是否提升|cFFFFFF00铭文|r技能？", 0 },
        { FUNC, "提升熟练度->|cFF006400【烹饪】|r->|cFFB22222450|r", Stone.SY12, GOSSIP_ICON_TRAINER, false, "是否提升|cFFFFFF00烹饪|r技能？", 0 },
        { FUNC, "提升熟练度->|cFF006400【急救】|r->|cFFB22222450|r", Stone.SY13, GOSSIP_ICON_TRAINER, false, "是否提升|cFFFFFF00急救|r技能？", 0 },
        { FUNC, "提升熟练度->|cFF006400【钓鱼】|r->|cFFB22222450|r", Stone.SY14, GOSSIP_ICON_TRAINER, false, "是否提升|cFFFFFF00钓鱼|r技能？", 0 },
    },

    [GMMENU] = {--GM菜单
        { FUNC, "直升80", Stone.LevelUp80, GOSSIP_ICON_CHAT, false, "是否更改名字？\n|cFFFFFF00需要重新登录才能修改。|r" },
        ...
        { FUNC, "重置副本", Stone.UnBind, GOSSIP_ICON_TRAINER, false, "确认重置副本?" },
        { FUNC, "提升熟练度->|cFF006400【采矿】|r->|cFFB22222450|r", Stone.SY01, GOSSIP_ICON_TRAINER, false, "是否提升|cFFFFFF00采矿|r技能？", 0 }, --增加收费金额参数,默认情况下失败也会扣金币，因此在失败时返还金币，在游戏里也不会出现减钱再加钱
        { FUNC, "提升熟练度->|cFF006400【草药】|r->|cFFB22222450|r", Stone.SY02, GOSSIP_ICON_TRAINER, false, "是否提升|cFFFFFF00草药|r技能？", 0 },
        { FUNC, "提升熟练度->|cFF006400【剥皮】|r->|cFFB22222450|r", Stone.SY03, GOSSIP_ICON_TRAINER, false, "是否提升|cFFFFFF00剥皮|r技能？", 0 },
        { FUNC, "提升熟练度->|cFF006400【工程】|r->|cFFB22222450|r", Stone.SY04, GOSSIP_ICON_TRAINER, false, "是否提升|cFFFFFF00工程|r技能？", 0 },
    ...
    [TPMENU] = {--主菜单
        ...
        { MENU, "传送到技能训练|cFF006400[武器/骑术/飞行/专业]|r", MMENU + 0x30, GOSSIP_ICON_TAXI },
    ...
    [TPMENU + 0x40] = {--卡利姆多
        ...
        { TP, "灰谷", 1, 1919.77, -2169.68, 94.6729, 6.14177, TEAM_NONE, 18, 20000 },
    ...
    [TPMENU + 0x70] = {--经典旧世界地下城★
        ...
        { TP, "阿塔哈卡神庙", 0, -10177.9, -3994.9, -111.239, 6.01885, TEAM_NONE, 35, 20000 },
    ...
    [TPMENU + 0xa0] = {--团队地下城★
        ...
        { TP, "|cFF0000FF【40人】奥妮克希亚的巢穴|r", 1, -4708.27, -3727.64, 54.5589, 3.72786, TEAM_NONE, 50, 100000 },
        ...
        { TP, "|cFF0000FF【40人】纳克萨玛斯|r", 329, 3929.59, -3380.76, 119.68, 1.629702, TEAM_NONE, 60, 100000 },
        ...
        { TP, "|cFF4B0082【10人】卡拉赞|r", 0, -11119.11, -2011.15, 47.0847, 0.695068, TEAM_NONE, 68, 100000 },
        ...
        { TP, "|cFF4B0082【25人】海加尔峰|r", 1, -8177.89, -4181.23, -167.552, 0.913338, TEAM_NONE, 70, 100000 },
        ...
        { TP, "|cFFB22222【10人/25人】红玉圣殿|r", 571, 3590.70, 210.56, -120.146, 5.350949, TEAM_NONE, 80, 100000 },
    },
    ...
    [MMENU + 0x20] = {--联盟职业技能训练师
        ...
        { TP, "萨满祭司训练师", 0, -9032.573242, 545.842590, 72.160950, 0, TEAM_ALLIANCE },
        { TP, "猎人训练师", 0, -8422.097656, 550.078674, 95.448730, 0, TEAM_ALLIANCE },
        { TP, "德鲁伊训练师", 1, 7870.23, -2586.97, 486.95, 0, TEAM_ALLIANCE },
        { TP, "潜行者训练师", 0, -8751.876953, 381.321930, 101.056236, 0, TEAM_ALLIANCE },
        ...
        { TP, "萨满祭司训练师", 1, 1928.482, -4228.17, 42.3219, 0, TEAM_HORDE },
        { TP, "猎人训练师", 1, 2135.33, -4610.78, 54.3865, 0, TEAM_HORDE },
        { TP, "德鲁伊训练师", 1, 7870.23, -2586.97, 486.95, 0, TEAM_HORDE },
        { TP, "潜行者训练师", 1, 1776.47, -4285.65, 7.44, 0, TEAM_HORDE },
    ...
    [MMENU + 0x30] = {--专业技能训练师
        ...
        { TP, "飞行训练师", 530, -4012.119873, 2165.176270, 104.866119, 0, TEAM_ALLIANCE },
        { TP, "武器训练师", 1, 2093.829346, -4821.349609, 24.382, 0, TEAM_HORDE },
        { TP, "骑术训练师", 530, 9268.768555, -7508.026367, 38.09, 0, TEAM_HORDE },
        { TP, "飞行训练师", 530, -3023.000000, 2613.000000, 76.905724, 0, TEAM_HORDE },
        -- {TP, "专业技能训练", 530, -2265.09, 5564.71, 67.01, 0,	TEAM_NONE},
    },
    [BUYMENU] = {-- 商人
        { FUNC, "新手商人", ST.SummonNPC_4003004, GOSSIP_ICON_VENDOR },
        -- {FUNC, "旅行商人", 	ST.SummonGNPC,	GOSSIP_ICON_VENDOR},
        -- {FUNC, "旅行修补匠", 	ST.SummonENPC,	GOSSIP_ICON_VENDOR},
        { FUNC, "召唤材料商-元素", ST.SummonNPC_4001001, GOSSIP_ICON_VENDOR },
        { FUNC, "召唤材料商-布料", ST.SummonNPC_4001002, GOSSIP_ICON_VENDOR },
        { FUNC, "召唤材料商-皮革", ST.SummonNPC_4001003, GOSSIP_ICON_VENDOR },
        { FUNC, "召唤材料商-金属和矿石", ST.SummonNPC_4001004, GOSSIP_ICON_VENDOR },
        { FUNC, "召唤材料商-草药", ST.SummonNPC_4001005, GOSSIP_ICON_VENDOR },
        { FUNC, "召唤材料商-附魔", ST.SummonNPC_4001006, GOSSIP_ICON_VENDOR },
        { FUNC, "召唤材料商-珠宝原石", ST.SummonNPC_4001007, GOSSIP_ICON_VENDOR },
        { FUNC, "召唤材料商-珠宝成品", ST.SummonNPC_4001008, GOSSIP_ICON_VENDOR },
        { FUNC, "召唤材料商-节日玩具1", ST.SummonNPC_4001009, GOSSIP_ICON_VENDOR },
        { FUNC, "召唤材料商-节日玩具2", ST.SummonNPC_4001010, GOSSIP_ICON_VENDOR },
        { MENU, "召唤材料商-雕文", BUYMENU + 0x10, GOSSIP_ICON_VENDOR },
        { MENU, "召唤材料商-钥匙", BUYMENU + 0x20, GOSSIP_ICON_VENDOR },
    },
    [BUYMENU + 0x10] = {-- 雕文
        { FUNC, "召唤雕文商-潜行者", ST.SummonNPC_4002001, GOSSIP_ICON_VENDOR },
        { FUNC, "召唤雕文商-德鲁伊", ST.SummonNPC_4002002, GOSSIP_ICON_VENDOR },
        { FUNC, "召唤雕文商-法师", ST.SummonNPC_4002003, GOSSIP_ICON_VENDOR },
        { FUNC, "召唤雕文商-猎人", ST.SummonNPC_4002004, GOSSIP_ICON_VENDOR },
        { FUNC, "召唤雕文商-牧师", ST.SummonNPC_4002005, GOSSIP_ICON_VENDOR },
        { FUNC, "召唤雕文商-圣骑士", ST.SummonNPC_4002006, GOSSIP_ICON_VENDOR },
        { FUNC, "召唤雕文商-萨满祭司", ST.SummonNPC_4002007, GOSSIP_ICON_VENDOR },
        { FUNC, "召唤雕文商-术士", ST.SummonNPC_4002008, GOSSIP_ICON_VENDOR },
        { FUNC, "召唤雕文商-死亡骑士", ST.SummonNPC_4002009, GOSSIP_ICON_VENDOR },
        { FUNC, "召唤雕文商-战士", ST.SummonNPC_4002010, GOSSIP_ICON_VENDOR },
    },
    [BUYMENU + 0x20] = {-- 钥匙
        { FUNC, "召唤钥匙商1", ST.SummonNPC_4003001, GOSSIP_ICON_VENDOR },
        { FUNC, "召唤钥匙商2", ST.SummonNPC_4003002, GOSSIP_ICON_VENDOR },
        { FUNC, "召唤钥匙商3", ST.SummonNPC_4003003, GOSSIP_ICON_VENDOR },
    },
...
local function Enchanting(player, EncSpell, Eid, money)
    --附魔 (玩家,附魔效果,附魔位置)
    local ID = Eid
    ...
    if (Nowitem and Eid) then
        ...
        local WName = Nowitem:GetItemLink(locale)--物品链接
