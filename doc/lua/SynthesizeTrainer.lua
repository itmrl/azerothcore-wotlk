print(">>Script: SynthesizeTrainer loading...OK")
...
function GOODS.Book(event, player, unit)
    --显示菜单
...
function GOODS.Select(event, player, unit, sender, intid, code, menu_id)
    ...
    if (intid < 0x10) then
        ...
        unit:UpdateEntry(NPC_ID)
    end
end

RegisterCreatureGossipEvent(NPC_ID, 1, GOODS.Book)
RegisterCreatureGossipEvent(NPC_ID, 2, GOODS.Select)
